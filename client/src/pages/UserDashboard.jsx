...
const UserDashboard = () => {
    ...
    return (
        <div className="max-w-6xl mx-auto">
            <div className="bg-[#444346]/67 rounded-2xl shadow-md p-6 sm:p-8 mb-8  flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left gap-4 sm:gap-6">
                <div className="w-20 h-20 bg-white text-[#130F0F] rounded-full flex items-center justify-center text-3xl font-bold uppercase tracking-widest shrink-0">
                    {user?.name.charAt(0)}
                </div>
                <div className="flex flex-col items-center sm:items-start">
                    <h1 className="text-2xl sm:text-3xl font-extrabold text-[#2EF82B]/71 mb-2">Welcome, {user?.name}!</h1>
                    <p className="text-white flex items-center justify-center sm:justify-start gap-2">
                        <span className="w-2 h-2 rounded-full bg-[#2EF82B]/71"></span> User Dashboard
                    </p>
                </div>
            </div>
            ...
            {bookings.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {bookings.map((booking) => (
                        <div key={booking._id} className="bg-[#444346]/67 rounded-xl overflow-hidden shadow-md hover:shadow-md transition  flex flex-col">
                            <div className="p-6 border-b border-white flex-grow">
                                {booking.eventId ? (
                                    <>
                                        <div className="flex justify-between items-start mb-4">
                                            <h3 className="text-lg font-bold text-[#2EF82B]/71 leading-tight">{booking.eventId.title}</h3>
                                            <div className="flex flex-col gap-1 items-end">
                                                ...
                                                {booking.status !== 'cancelled' && (
                                                    <span className={`px-2 py-1 text-[10px] font-black rounded uppercase tracking-wider ${booking.paymentStatus === 'paid' ? 'bg-blue-100 text-blue-900' : 'bg-gray-100 text-gray-700'
                                                        }`}>
                            ...
                            <div className="p-4 bg-gray-50 flex justify-between items-center shrink-0">
                                {booking.eventId && booking.status !== 'cancelled' ? (
                                    <>
                                        <Link to={`/events/${booking.eventId._id}`} className="text-[#130F0F] font-semibold text-sm hover:underline">View Event</Link>
                                        <button
